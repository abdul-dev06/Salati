...
(function () {
  ...
  // Update icon when fullscreen state changes (e.g. user presses Escape)
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('/Salati/sw.js')
      .catch(err => console.warn('SW registration failed:', err));
  });
}

// ─── 3AM AUTO-RELOAD (clears memory for 24/7 displays) ───
(function () {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', (e) => {
    if (e.target.closest('#right-controls')) return;
    if (e.target.id === 'weather-toggle')    return;
    if (!reminderAudio.paused) { reminderAudio.pause(); reminderAudio.currentTime = 0; }
    if (!athanAudio.paused)    { athanAudio.pause();    athanAudio.currentTime = 0;    }
  }, { passive: true });
  ...
  (function () {
    ...
    btn.addEventListener('click', (e) => {
      ...
      // Stop any currently playing audio immediately when muted
      if (!soundEnabled) {
        if (!reminderAudio.paused) { reminderAudio.pause(); reminderAudio.currentTime = 0; }
        if (!athanAudio.paused)    { athanAudio.pause();    athanAudio.currentTime = 0;    }
      }
  ...
  function checkAudio() {
    ...
    for (const id of prayerIds) {
      ...
      // Reminder: fire once when 15 min window opens
  ...
  function startResilientPoller(fn, label, intervalMs, retryMs = RETRY_MS) {
    ...
    const run = async () => {
      ...
      try {
        ...
        console.warn(`Poller "${label}" failed, retrying...`, err);
        running = false;
